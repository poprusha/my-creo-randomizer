...
// const command = (creoPath, endPath, outputName) => `ffmpeg -i "${creoPath}" -i "${endPath}" -filter_complex "[0:v][0:a][1:v][1:a]concat=n=2:v=1:a=1[outv][outa]" -map "[outv]" -map "[outa]" -c:v libx264 -c:a aac "${outputName}"`;
    ...
    for (let i = 0; i < process.env.VIDEOS_COUNT; i++) {
        await getVideosList(CREO_PATH)
            .then(async (data) => {
                ...
                const creoName = data.filter(el => el.name.endsWith('.mp4')).map(el => el.name)[i];
